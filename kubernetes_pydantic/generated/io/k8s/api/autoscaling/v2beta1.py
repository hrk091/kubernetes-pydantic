...
class ExternalMetricSource(BaseModel):
    ...
    metric_name: str = Field(..., alias='metricName', description='metricName is the name of the metric in question.')
...
class ExternalMetricStatus(BaseModel):
    ...
    current_value: resource.Quantity = Field(
        ..., alias='currentValue', description='currentValue is the current value of the metric (as a quantity)'
    )
    metric_name: str = Field(
        ..., alias='metricName', description='metricName is the name of a metric used for autoscaling in metric system.'
    )
    metric_selector: Optional[v1.LabelSelector] = Field(
        None,
        alias='metricSelector',
...
class ObjectMetricSource(BaseModel):
    ...
    target: CrossVersionObjectReference = Field(..., description='target is the described Kubernetes object.')
    target_value: resource.Quantity = Field(
        ..., alias='targetValue', description='targetValue is the target value of the metric (as a quantity).'
    )
...
class HorizontalPodAutoscalerSpec(BaseModel):
    ...
    metrics: Optional[List[MetricSpec]] = Field(
        None,
        description='metrics contains the specifications for which to use to calculate the desired replica count (the maximum replica count across all metrics will be used).  The desired replica count is calculated multiplying the ratio between the target value and the current value by the current number of pods.  Ergo, metrics used must decrease as the pod count is increased, and vice-versa.  See the individual metric source generated for more information about how each type of metric must respond.',
    )

...
class FlowDistinguisherMethod(BaseModel):
    ...
    type: str = Field(
        ...,
        description='`type` is the type of flow distinguisher method The supported generated are "ByUser" and "ByNamespace". Required.',
    )
...
class QueuingConfiguration(BaseModel):
    ...
    queue_length_limit: Optional[int] = Field(
        None,
...
class UserSubject(BaseModel):
    ...
    name: str = Field(..., description='`name` is the username that matches, or "*" to match all usernames. Required.')
...
class FlowSchemaCondition(BaseModel):
    ...
    message: Optional[str] = Field(
        None, description='`message` is a human-readable message indicating details about last transition.'
    )
    reason: Optional[str] = Field(
        None, description="`reason` is a unique, one-word, CamelCase reason for the condition's last transition."
    )
    status: Optional[str] = Field(
        None, description='`status` is the status of the condition. Can be True, False, Unknown. Required.'
    )
...
class LimitedPriorityLevelConfiguration(BaseModel):
    ...
    assured_concurrency_shares: Optional[int] = Field(
        None,
        alias='assuredConcurrencyShares',
    ...
    limit_response: Optional[LimitResponse] = Field(
        None,
        alias='limitResponse',
...
class Subject(BaseModel):
    ...
    group: Optional[GroupSubject] = Field(None, description='`group` matches based on user group name.')
    kind: str = Field(..., description='`kind` indicates which one of the other fields is non-empty. Required')
    service_account: Optional[ServiceAccountSubject] = Field(
        None, alias='serviceAccount', description='`serviceAccount` matches ServiceAccounts.'
    )
...
class PolicyRulesWithSubjects(BaseModel):
    ...
    non_resource_rules: Optional[List[NonResourcePolicyRule]] = Field(
        None,
    ...
    resource_rules: Optional[List[ResourcePolicyRule]] = Field(
        None,
        alias='resourceRules',
...
class PriorityLevelConfigurationList(BaseModel):
    ...
    items: List[PriorityLevelConfiguration] = Field(..., description='`items` is a list of request-priorities.')
    kind: Optional[str] = Field(
        None,
        description='Kind is a string value representing the REST resource this object represents. Servers may infer this from the endpoint the client submits requests to. Cannot be updated. In CamelCase. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds',
    )
...
class FlowSchemaSpec(BaseModel):
    ...
    distinguisher_method: Optional[FlowDistinguisherMethod] = Field(
        None,
    ...
    matching_precedence: Optional[int] = Field(
        None,
